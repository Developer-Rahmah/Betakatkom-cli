...
export default ({ type, data, isGrid, parentType, index, id }: CardProps) => {
    const styles = StyleSheet.create({
        card: {
            width: isGrid ? CARD_WIDTH / 1.8 : CARD_WIDTH,
            height: isGrid ? CARD_HEIGHT / 1.95 : CARD_HEIGHT / 1.7,
            marginVertical: 15,
    ...
    const onNavigation = (data) => {
        ...
        if (allData[type] != undefined) {
            navigation.navigate('CardListingScreen', { type: type, source: source, items: data.children, title: data.nameArabic, id: data.id, parentType: parentType, groupID: id })
    ...
    return <TouchableOpacity style={styles.button} onPress={() => onNavigation(data)}>
        <ImageBackground borderRadius={15} style={styles.card} source={source ? source : { uri: data.image }} >
            <View style={{ backgroundColor: data.nameEnglish === 'Egyptian Cards' ? Colors.BLACK : 'transparent', opacity: 0.7, marginTop: 0, width: '90%', height: '50%', justifyContent: 'center', alignItems: 'center' }}>
                {data.id == 'Egyptian Cards' ?
                    <Title fontFamily='Cairo-Bold' style={{ fontSize: 25, marginTop: -5, color: Colors.WHITE }} title={data.nameArabic} />
                    :
                    data.id === 234 ?
                        <View style={{ width: '80%', height: 50, backgroundColor: Colors.BLACK, opacity: .9, justifyContent: 'center', alignItems: 'center' }}>
                            <Title fontFamily='Cairo-Bold' style={{ fontSize: 25, marginTop: -7, color: Colors.WHITE }} title={data.nameArabic} />
                        </View>
                        :
                        data.id === 10 || data.id === 11 || data.id === 12 || data.id === 109 ?
                            <View style={{ width: isGrid ? '80%' : '100%', height: 50, backgroundColor: Colors.BLACK, opacity: .9, justifyContent: 'center', alignItems: 'center' }}>
                                <Title fontFamily='Cairo-Bold' style={{ fontSize: 25, marginTop: -7, color: Colors.WHITE }} title={data.nameArabic} />
                            </View>
                            :
                            <>
                                ...
                                {
                                    !isGrid &&
                                    <View style={{ width: '100%', height: 50, backgroundColor: Colors.BLACK, opacity: .9, justifyContent: 'center', alignItems: 'center' }}>
                                        <Title fontFamily='Cairo-Bold' style={{ fontSize: 25, marginTop: -7, color: Colors.WHITE }} title={data.nameArabic} />
                                    </View>
                                }
                            </>
